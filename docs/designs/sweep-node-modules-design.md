...
本总纲只承载项目级设计 (定位、结构、测试、全局边界); 各「不可再拆分的设计块」独立成册:

| 分册                                         | 承载                                           |
| -------------------------------------------- | ---------------------------------------------- |
| [命令面与输出](cli-surface.md)               | 命令、旗标、退出码、清单渲染规格               |
| [配置与初始化](config-and-initialization.md) | 配置文件规格、roots / exclude 语义、初始化向导 |
| [扫描与体积](scan-and-size.md)               | 遍历算法、剪枝与排除、体积统计、性能要点       |
| [删除安全闸](deletion-guard.md)              | 删除校验不变量与执行语义                       |
