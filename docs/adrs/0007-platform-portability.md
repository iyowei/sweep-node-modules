...
4. **入口分轨**: macOS / Linux 用 `bin/sweep-nm` (sh); Windows 用 `bin/sweep-nm.cmd` (同逻辑的 cmd 包装, Bun 优先 / Node 回退, 不用 `readlink`); 包管理器 bin shim 亦兼容。Windows 两项 (cmd 启动器实跑 / shim 兼容) 均未经真机验证 `[证据缺口]`, 待真机轮补验。
...
- 体积统计失去「平台命令」的免费加速, 需接受纯实现性能或维护双轨 (已裁定为策略 A 双轨, 见 [扫描与体积](../designs/scan-and-size.md));
- 入口与颜色层多一档平台分支。
...
3. 体积统计两候选的基准数据已产出 (du 快路径合成场景约 11× 于纯实现, 双候选行为契约全绿); 裁定为**策略 A 双轨**, 已回写 [扫描与体积](../designs/scan-and-size.md)。
