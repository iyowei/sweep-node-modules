...
- 回归需在两种运行时下各跑一遍 (测试命令双跑; 修订 (2026-09-23): 单测侧此后已参数化, e2e 按两个载体各注册一遍, 跑一次 `bun test` 即覆盖两侧 (前提是机器上两个运行时都装, 缺哪一侧则该侧用例静默 skip 而整体仍全绿), 故「双跑」现仅对**转写验收**通道成立, 其 `--target` 为单值且不在任何闸门内);
