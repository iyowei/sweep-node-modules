...
1. **钩子框架 lefthook**, `prepare` 按 CI 安全形态安装: `[ -z "$CI" ] && lefthook install || true`。
...
2. `bunx tsc --noEmit`、`bunx oxlint`、`bunx prettier --check` 全绿; `bun test` 全量全绿 (实现落地后 243 条, 含双载体 e2e 与伪终端冒烟);
