...
- **单测**已参数化: e2e 用例按 `bun` / `node` 各注册一遍, 跑一次 `bun test` 即覆盖两侧, 且它在 pre-push 闸门内。**前提是机器上两个运行时都装了**: 缺哪一侧, 那一侧的用例会静默 skip, 整体仍显示全绿 (该 skip 是有意设计, 见 `src/runtime.test.ts` 文件头), 故 bun-only 机器上闸门不构成 node 侧的把关;
