...
> **终稿数据**: 双载体 (bun / node) 各三连跑 40/40 全绿, 判定层逐字节一致;
> 判定基准为 14:17 快照 (`cli.ts` 删除安全复核适配后);
> 可追溯校验 (jq): 台账 48 条 / 语料引用 40 条 / 悬空引用 0, 未被引用者恰为下表 8 条豁免项。
...
| OF-03 | 10     | delete-execute-ok, render-align-cjk, render-path-tilde, scan-basic-preview, scan-exclude-cli-merge, scan-exclude-config, scan-git-bait, scan-nested-prune, scan-root-symlink-followed, scan-symlink-not-followed                                              |
...
| OF-12 | 6      | cli-help, cli-unknown-arg, scan-basic-preview, scan-exclude-unmatched-warn, scan-root-missing-warn, scan-unreadable-dir-warn                                                                                                                                  |
...
完全一致 (23 / 11 / 3 / 2 / 5 / 23), 抓取面稳定。

| mutant (注入缺陷)                | 抓住它的用例数 | 代表用例                                                                |
| -------------------------------- | -------------- | ----------------------------------------------------------------------- |
| prune-negated (剪枝谓词取反)     | 23             | scan-basic-preview, scan-nested-prune, render-tier-mid-and-order        |
| exit-swallowed (退出码吞掉)      | 11             | cli-unknown-arg, config-corrupt-json, size-unmeasured-blocks-delete     |
| sort-missing (排序缺失)          | 3              | scan-order-target-asc, scan-basic-preview, delete-execute-multi-summary |
| exclude-silent (排除静默失效)    | 2              | scan-exclude-config, scan-exclude-cli-merge                             |
| message-removed (提示语删改)     | 5              | render-empty-result, render-banner-4-roots, scan-root-is-node-modules   |
| size-unit-wrong (体积计数单位错) | 23             | render-tier-mid-and-order, scan-basic-preview, size-unmeasured-preview  |

观察: `sort-missing` 抓取面最窄 (3 条), 因它依赖「并发完成序 ≠ 升序」; 语料以 3 项同体积清单
(`scan-order-target-asc`) 作主抓点, 抓取稳定 (两轮重建 mutant 均被抓)。
